...
CustomArray& CustomArray::NewBlock(CustomCell* previouscell, unsigned long size)
{
	...
	Cell->Item.Addy = (void*)new uint8_t[Cell->Item.Max];
	Cell->Item.Size = 0;
...
void* CustomArray::Collapse(void* userbuffer)
{
	...
	if(!userbuffer)
	{
		...
		mCollapsed = CurrentSize ? new uint8_t[CurrentSize] : null;
...
bool CustomArray::PushAddress()
{
	...
	if((mNbPushedAddies+1)>mNbAllocatedAddies)
	{
		...
		uint32_t NewSize = mNbAllocatedAddies ? mNbAllocatedAddies * 2 : 1;
...
// Output	:	-
// Return	:	uint32_t	= size in bytes, or 0 if file doesn't exist
// Exception:	-
// Remark	:	-
#ifndef SEEK_END
#define SEEK_END 2
#endif
uint32_t CustomArray::FileSize(const char* name)
{
	FILE* File;
	errno_t err;
